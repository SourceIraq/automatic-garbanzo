...
-قـم بأرسال `.الاوامر`
''')
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.مطور"))
async def _(event):
    start = datetime.datetime.now()
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك العراق ابشر @Source_elesyed')
